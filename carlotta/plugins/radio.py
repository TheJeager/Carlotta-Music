...
RADIO_STATIONS: dict[str, tuple[str, str, str]] = {
    ...
    "ndr2": (
        "NDR 2",
        "Germany",
        "https://icecast.ndr.de/ndr/ndr2/hamburg/mp3/128/stream.mp3",
    ),
    "deejay": (
        "Radio Deejay",
        "Italy",
        "https://radiodeejay-lh.akamaihd.net/i/RadioDeejay_1@347065/master.m3u8",
    ),
    ...
    "kissfm": (
        "Kiss FM",
        "United Kingdom",
        "https://stream-kiss.planetradio.co.uk/kissnational.mp3",
    ),
    "capitalfm": (
        "Capital FM",
        "United Kingdom",
    ...
    "planetrock": (
        "Planet Rock",
        "United Kingdom",
        "https://media-ice.musicradio.com/PlanetRockMP3",
    ),
    "abcnews": (
        "ABC News Radio",
        "Australia",
        "https://mediaserviceslive.akamaized.net/hls/live/2038315/newsradio/masterhq.m3u8",
    ),
    "tripledj": (
        "triple j",
        "Australia",
    ...
    "doublej": (
        "Double J",
        "Australia",
        "https://live-radio01.mediahubaustralia.com/DJDW/mp3/",
    ),
    "radionz": (
        "RNZ National",
        "New Zealand",
        "https://radionz-ice.streamguys.com/national.mp3",
    ),
    "rne1": (
        "RNE Radio Nacional",
        "Spain",
    ...
    "slam": (
        "SLAM!",
        "Netherlands",
        "https://22543.live.streamtheworld.com/WEB12_MP3_SC",
    ),
    "qmusicnl": (
        "Qmusic",
        "Netherlands",
    ...
    "ylex": (
        "YleX",
        "Finland",
        "https://yleuniicodnaudio-live.akamaized.net/hls/live/617138/ylxfin/playlist.m3u8",
    ),
    "polskieradio": (
        "Polskie Radio Jedynka",
        "Poland",
        "https://stream4.nadaje.com:8348/radio1.mp3",
    ),
    "radiojavan": ("Radio Javan", "Iran", "https://stream.radiojavan.com/radiojavan"),
    "redfm": (
        "Red FM 93.5",
        "India",
        "https://n02.radiojar.com/6phb7r0x7vzuv?rj-ttl=5&rj-tok=AAABiZz6QfEAi0CYOacSLbLhSA",
    ),
    "radiomirchi": (
        "Radio Mirchi Hindi",
        "India",
...
def _by_country(country: str) -> list[tuple[str, str]]:
    return sorted(
        [(c, n) for c, (n, k, _) in RADIO_STATIONS.items() if k == country],
        key=lambda x: x[1],
    )
...
def _country_markup(page: int = 0) -> types.InlineKeyboardMarkup:
    ...
    rows = [
        [types.InlineKeyboardButton(f"🌍 {c}", callback_data=f"radio country {c} 0")]
        for c in chunk
    ]
    nav = []
    if page > 0:
        nav.append(
            types.InlineKeyboardButton("◀️", callback_data=f"radio countries {page - 1}")
        )
    nav.append(
        types.InlineKeyboardButton(
            f"{page + 1}/{max(1, (len(countries) - 1) // PAGE_SIZE + 1)}",
    ...
    rows.append(
        [
            types.InlineKeyboardButton(
                "🇮🇳 India Stations", callback_data="radio country India 0"
            )
        ]
    )
...
def _station_markup(country: str, page: int = 0) -> types.InlineKeyboardMarkup:
    ...
    rows = [
        [types.InlineKeyboardButton(f"🎧 {name}", callback_data=f"radio play {code}")]
        for code, name in chunk
    ]
    nav = [
        types.InlineKeyboardButton("🔙 Countries", callback_data="radio countries 0")
    ]
    if page > 0:
        nav.append(
            types.InlineKeyboardButton(
                "◀️", callback_data=f"radio country {country} {page - 1}"
            )
        )
...
@app.on_message(
    filters.command(["radio", "station", "stations"]) & filters.group & ~app.bl_users
)
@lang.language()
async def radio_handler(_, m: types.Message) -> None:
    ...
    force = arg1 == "force"
    station_code = (
        m.command[2].lower().strip() if force and len(m.command) > 2 else arg1
    )
    await _play_station(_, m, station_code, force=force)


async def _play_station(
    _, m: types.Message, station_code: str, force: bool = False
) -> None:
    station = RADIO_STATIONS.get(station_code)
    if not station:
        return await m.reply_text(
            "❌ Unknown station code. Use <code>/radio</code> to browse all stations."
        )
...
async def radio_callback(_, query: types.CallbackQuery) -> None:
    ...
    if action == "countries":
        ...
        await query.message.edit_text(
            _radio_help_text(), reply_markup=_country_markup(page)
        )
        return await query.answer()
