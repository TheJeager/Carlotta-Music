...
class Thumbnail:
    def __init__(self):
        try:
            self.font_title = ImageFont.truetype(
                "carlotta/helpers/Raleway-Bold.ttf", 58
            )
            self.font_artist = ImageFont.truetype(
                "carlotta/helpers/Inter-Light.ttf", 32
            )
    ...
    def get_palette(self, source):
        ...
        accent = clamp(
            top_colors[4] if len(top_colors) > 4 else top_colors[-1], min_channel=64
        )
    ...
    def draw_text_with_shadow(
        self, draw, xy, text, font, fill, shadow=(0, 0, 0, 145), offset=(0, 2)
    ):
    ...
    async def generate(self, song: Track, size=(1280, 720)):
        try:
            ...
            thumb = (
                Image.open(temp).convert("RGBA").resize(size, Image.Resampling.LANCZOS)
            )
            ...
            image = Image.alpha_composite(
                image, self.make_vertical_gradient(size, top_tint, bottom_tint)
            )
            ...
            hdraw.rounded_rectangle(
                (118, 86, 1162, 176), radius=34, fill=(255, 255, 255, 16)
            )
            hdraw.rounded_rectangle(
                (118, 86, 1162, 176), radius=34, outline=(255, 255, 255, 40), width=1
            )
            ...
            content = ImageOps.fit(
                thumb, (inner_w, inner_h), method=Image.Resampling.LANCZOS
            )
            ...
            artist = self.truncate(
                song.channel_name or "Unknown Artist", self.font_artist, info_w, draw
            )
            album = self.truncate(
                self.format_album(song), self.font_album, info_w, draw
            )
            ...
            self.draw_text_with_shadow(
                draw, (info_x, 338), artist, self.font_artist, (239, 239, 239, 230)
            )
            self.draw_text_with_shadow(
                draw, (info_x, 388), album, self.font_album, (220, 220, 220, 216)
            )
            ...
            draw.rounded_rectangle(
                [bar_x1, bar_y1, bar_x2, bar_y2], radius=8, fill=(255, 255, 255, 76)
            )
            progress_ratio = 0
            ...
            if progress_x > bar_x1:
                draw.rounded_rectangle(
                    [bar_x1, bar_y1, progress_x, bar_y2], radius=8, fill=(*accent, 232)
                )
