...
from carlotta import app, config, db, logger, queue, yt
from carlotta.helpers import buttons, utils, voice_optimizer
...
def checkUB(play):
    async def wrapper(_, m: types.Message, *args, **kwargs):
        ...
        video = call_kwargs.pop("video", None)
        if video is None:
            ...
            try:
                ...
                if m.chat.username:
                    ...
                    try:
                        ...
                        voice_optimizer.set_cached_invite(chat_id, invite_link)
                    except errors.ChatAdminRequired:
                        return await m.reply_text(m.lang["admin_required"])
                ...
                try:
                    await voice_optimizer.fast_join(client, chat_id, invite_link)
